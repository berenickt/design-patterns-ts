...
/**
 * Some support interfaces and classes
 */
interface Country {
  name: string
  vat: number
...
class HtmlInvoiceFormatter extends InvoiceFormatter {
  ...
  protected formatInvoiceLines(): string {
    return `<ul>\n${this.invoice.lines
      .map(l => `  <li>${l.product}, units=${l.units}, amount=${l.units * l.unitPrice}€</li>`)
      .join('\n')}</ul>`
  }
...
class MarkdownInvoiceFormatter extends InvoiceFormatter {
  ...
  protected formatInvoiceLines(): string {
    return `${this.invoice.lines
      .map(l => `*${l.product}, units=${l.units}, amount=${l.units * l.unitPrice}€`)
      .join('\n')}`
  }
  protected formatTaxes(): string {
