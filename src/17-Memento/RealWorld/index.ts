...
class EmployeeOriginator {
  ...
  public showState(): void {
    console.log(
      `Employee: State for ${this.name} is salary=${this._salary} and monthlyExpensesLimit=${this._monthlyExpensesLimit}`,
    )
  }
}

/**
 * The Memento interface provides provides some metadata, apart from the
 ...
 * monthlyExpensesLimit in the memento constructor and moving the restore method
 * to the memento class instead of the originator
 */
interface Memento<T> {
  readonly state: T
...
/**
 * We also declare an interface for the employee (originator) state. Not all the
 * employee fields should be in the state.
 */
